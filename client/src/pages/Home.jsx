...
export default function Home({ onLogout }) {
  ...
  useEffect(() => {
    ...
    api.get(`/api/users/${user._id}/friends`)
      .then((res) => {
        setFriends(res.data);
        const friendIds = res.data.map((f) => f._id);
        api.get('/api/posts').then((postRes) => {
          const filtered = postRes.data.filter((post) => friendIds.includes(post.userId));
          setPosts(filtered);
        });
      });

    api.get(`/api/users/${user._id}/friend-requests`)
      .then((res) => setFriendRequests(res.data));

    api.get('/api/users').then((res) => {
      setUsers(res.data);

      const requestedTo = res.data
        .filter((u) => u.friendRequests.includes(user._id))
        .map((u) => u._id);
  ...
  const acceptRequest = async (senderId) => {
    await api.post(`/api/users/${currentUser._id}/accept-request`, { senderId });
    const updatedFriends = await api.get(`/api/users/${currentUser._id}/friends`);
    const updatedRequests = await api.get(`/api/users/${currentUser._id}/friend-requests`);
    setFriends(updatedFriends.data);
    setFriendRequests(updatedRequests.data);
  };

  const sendFriendRequest = async (targetId) => {
    await api.post(`/api/users/${currentUser._id}/send-request`, { targetId });
    setSentRequests([...sentRequests, targetId]);
  };

  const cancelFriendRequest = async (targetId) => {
    const confirmCancel = window.confirm('Cancel friend request?');
    if (!confirmCancel) return;

    await api.post(`/api/users/${currentUser._id}/cancel-request`, { targetId });
              ...
              {users
                .filter(
                  (u) =>
                    ...
                    !confirmedFriendIds.includes(u._id)
                )
                .map((user) => (
