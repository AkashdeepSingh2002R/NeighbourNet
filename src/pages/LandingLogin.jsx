...
export default function LandingLogin({ onLogin }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (isSignup) {
      ...
      const emailExists = allUsers.some((user) => user.email === trimmedEmail);
      if (emailExists) {
      ...
      localStorage.setItem('user', JSON.stringify(newUser)); // session
      onLogin(newUser);
      navigate('/home');
    } else {
      ...
      if (foundUser) {
        localStorage.setItem('user', JSON.stringify(foundUser)); // session
        onLogin(foundUser);
        navigate('/home');
      } else {
